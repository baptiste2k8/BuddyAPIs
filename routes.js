...
router.get('/buddy_actions_Json', async function (req, res) {
  try {
    // Check if the user is logged in
    //if (!req.session.userId) {
      // User is not logged in, redirect to the login page
...
// Add wimp route
router.post('/buddy_actions/add', async (req, res) => {
...
try {
  ...
  console.log(`Unique ID: ${req.body.receivedOn}`);
...
router.post('/buddy_actions/addAPI', async (req, res) => {
  console.log('into buddy add API');
const { uniqueID,receivedOn,buddyPayload,buddyPayloadExecutionStatus,buddyPayloadExecutionOutcome,triggeringEvent } = req.body;
try {
  await db.query('INSERT INTO buddy_actions (uniqueID, receivedOn,buddyPayload, buddyPayloadExecutionStatus, buddyPayloadExecutionOutcome,triggeringEvent) VALUES (?,?, ?, ?, ?,?)', [
  uniqueID,
...
// Delete wimp route
router.delete('/buddy_actions/delete/:uniqueID', async (req, res) => {
console.log('into buddy_actions/delete');
